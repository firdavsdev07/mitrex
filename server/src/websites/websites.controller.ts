...
export class WebsitesController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Website created with a unique tracking key',
  })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get a single website (metadata only)' })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiResponse({ status: 200, description: 'Website metadata' })
  async findOne(@CurrentUser('id') userId: string, @Param('id') id: string) {
    const w = await this.websitesService.findOne(userId, id);
    return {
      id: w.id,
      name: w.name,
      domain: w.domain,
      trackingKey: w.trackingKey,
      shareId: w.shareId,
  ...
  @ApiResponse({
    status: 200,
    description: 'HTML script tag to embed on the site',
  })
  ...
  @ApiOperation({
    summary: 'Get daily visitors + pageviews trend for charting',
  })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiQuery({
    name: 'period',
  ...
  @ApiResponse({
    status: 200,
    description: 'Daily { date, visitors, views }[]',
  })
  getTrend(
  ...
  @Get(':id/analytics')
  @ApiOperation({
    summary: 'Get overview analytics (visitors, views, bounce rate)',
  })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiQuery({
    name: 'period',
  ...
  @ApiResponse({
    status: 200,
    description: 'Real-time visitor count and pages',
  })
  ...
  async getEvents(
    ...
    await this.websitesService.findOne(userId, id);
  ...
  @ApiOperation({
    summary:
      'Compute a conversion funnel from an ordered list of custom event names',
  })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiQuery({
    name: 'period',
  ...
  @ApiResponse({
    status: 200,
    description: 'Per-step counts, drop-off %, overall conversion %',
  })
  getFunnel(
    @CurrentUser('id') userId: string,
    @Param('id') id: string,
    ...
    return this.websitesService.getFunnel(userId, id, steps, period);
  }

  @Get(':id/events/:name')
  @ApiOperation({
    summary: 'Get recent event instances for a specific event name',
  })
  @ApiParam({ name: 'id', description: 'Website ID' })
  ...
  @ApiResponse({
    status: 200,
    description: 'Last 20 event instances with properties',
  })
  ...
  @Post(':id/share')
  @ApiOperation({ summary: 'Enable a public (unauthenticated) dashboard link' })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiResponse({
    status: 200,
    description: '{ shareId } — use with GET /public/dashboard/:shareId',
  })
  enableShare(@CurrentUser('id') userId: string, @Param('id') id: string) {
    return this.websitesService.enableShare(userId, id);
  }

  @Delete(':id/share')
  @ApiOperation({ summary: 'Disable the public dashboard link' })
  @ApiParam({ name: 'id', description: 'Website ID' })
  @ApiResponse({ status: 200, description: '{ disabled: true }' })
  disableShare(@CurrentUser('id') userId: string, @Param('id') id: string) {
    return this.websitesService.disableShare(userId, id);
  }
}
