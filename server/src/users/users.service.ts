...
export class UsersService {
  ...
  async deleteAccount(userId: string, dto: DeleteAccountDto) {
    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user) throw new NotFoundException('User not found');
    if (user.deletedAt) throw new BadRequestException('Hisob allaqachon o\'chirilgan');

    const restoreToken = crypto.randomBytes(32).toString('hex');
    const restoreTokenExpiresAt = new Date(Date.now() + 180 * 24 * 60 * 60 * 1000); // 6 months
    ...
    await this.email.sendAccountDeleted(user.email, user.name || '', restoreUrl);

    return { message: 'Hisob o\'chirildi. 6 months ichida tiklashingiz mumkin.' };
  }
  ...
  async restoreAccount(token: string) {
    ...
    if (!user) throw new BadRequestException('Token noto\'g\'ri yoki muddati tugagan');
    if (!user.restoreTokenExpiresAt || user.restoreTokenExpiresAt < new Date()) {
