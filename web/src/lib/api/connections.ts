...
export const connectionsApi = {
  ...
  disconnect: (platform: string) =>
    apiClient.delete(`/connections/${platform}`).then((r) => r.data),

  syncOne: (platform: string) =>
    apiClient.post(`/connections/${platform}/sync`).then((r) => r.data),
...
export const PLATFORM_META: Record<
  string,
  { label: string; color: string; connectType: "oauth" | "youtube" | "bluesky" | "telegram" | "telegram_handle" | "manual" }
> = {
  YOUTUBE: { label: "YouTube", color: "text-red-400", connectType: "youtube" },
  TELEGRAM: { label: "Telegram", color: "text-blue-400", connectType: "telegram_handle" },
  INSTAGRAM: { label: "Instagram", color: "text-pink-400", connectType: "oauth" },
