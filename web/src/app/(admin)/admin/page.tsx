    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-28 rounded-xl border border-zinc-800 bg-zinc-900/60 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        <p className="text-xs text-zinc-600 uppercase tracking-wider mb-0.5">Admin</p>
        <h1 className="text-lg font-semibold text-zinc-100">Overview</h1>
        <p className="text-sm text-zinc-500 mt-1">Platform bo'yicha umumiy statistika</p>
      </div>
      ...
      <div className="rounded-xl border border-zinc-800/60 bg-zinc-900/30 p-5">
        <p className="text-sm text-zinc-500">
          ...
          <a href="/admin/users" className="text-zinc-300 hover:text-white underline underline-offset-2">
            Foydalanuvchilar
          </a>{" "}
          bo'limiga o'ting. Planlar va platformalarni sozlash uchun mos bo'limlardan foydalaning.
        </p>
      </div>
    </div>
  );
