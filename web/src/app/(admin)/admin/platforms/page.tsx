...
export default function AdminPlatformsPage() {
  ...
  useEffect(() => {
    adminApi.getPlatforms().then(setPlatforms).finally(() => setLoading(false));
  }, []);

  async function toggle(slug: string, field: "enabled" | "comingSoon", current: boolean) {
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-16 rounded-xl border border-zinc-800 bg-zinc-900/60 animate-pulse" />
          ))}
        </div>
      ) : platforms.length === 0 ? (
          ...
          <CardContent className="p-0">
            ...
            {platforms.map((p, i) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="text-sm font-medium text-zinc-200">{p.displayName}</p>
      ...
      <div className="mt-4 p-4 rounded-xl border border-zinc-800/60 bg-zinc-900/30">
        <p className="text-xs text-zinc-600">
          <span className="text-green-400">Faol</span> — foydalanuvchilar ulana oladi. {" "}
          <span className="text-orange-400">Tez kunda</span> — platforma ko'rinadi lekin ulash mumkin emas.
        </p>
      </div>
    </div>
  );
