...
import {
  Plus, Bot, Globe, Link2, Trash2,
  TrendingUp, TrendingDown, BarChart2,
  CalendarDays, Calendar, FileText, X,
  Sparkles, Clock,
} from "lucide-react";
...
import { dashboardApi, type TrendPoint, type PlatformHistoryPoint } from "@/lib/api/dashboard";
...
interface AiInsight {
  id: string;
  type: "WEEKLY" | "MONTHLY" | "YEARLY" | "DAILY" | "WEBSITE" | "POST" | "CUSTOM";
  title: string | null;
  content: string;
  provider: string;
...
const TYPE_LABELS: Record<string, string> = {
  WEEKLY: "Haftalik", MONTHLY: "Oylik", YEARLY: "Yillik",
  WEBSITE: "Sayt tahlili", CUSTOM: "Platforma tahlili",
  POST: "Post tahlili", DAILY: "Kunlik",
};

const TYPE_ICONS: Record<string, React.FC<{ className?: string }>> = {
  WEEKLY: BarChart2, MONTHLY: CalendarDays, YEARLY: Calendar,
  WEBSITE: Globe, CUSTOM: Link2, POST: FileText, DAILY: Clock,
};

const TYPE_COLOR: Record<string, string> = {
  WEEKLY: "text-orange-400", MONTHLY: "text-yellow-400",
  YEARLY: "text-purple-400", WEBSITE: "text-blue-400",
  CUSTOM: "text-green-400", POST: "text-zinc-400", DAILY: "text-zinc-400",
};

const PLATFORM_CHART_COLORS: Record<string, string> = {
  YOUTUBE: "#ff4444", TELEGRAM: "#2AABEE",
  INSTAGRAM: "#E1306C", DISCORD: "#5865F2", BLUESKY: "#0085ff",
};
...
function renderInline(text: string): React.ReactNode {
  return text.split(/(\*\*[^*]+\*\*|`[^`]+`)/g).map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`"))
      return <code key={i} className="text-orange-300 bg-zinc-800 px-1.5 py-0.5 rounded text-xs font-mono">{part.slice(1, -1)}</code>;
    return part;
  });
}
  ...
  return (
    <div className="text-[15px] text-zinc-300 leading-7 space-y-2">
      {text.split("\n").map((line, i) => {
        ...
        if (line.startsWith("## "))
          return <h2 key={i} className="text-lg font-bold text-zinc-100 mt-6 mb-2 first:mt-0">{renderInline(line.slice(3))}</h2>;
        if (line.startsWith("### "))
          return <h3 key={i} className="text-base font-semibold text-zinc-200 mt-4 mb-1.5 first:mt-0">{renderInline(line.slice(4))}</h3>;
        if (line.match(/^[-*]\s/))
          return <div key={i} className="flex gap-2.5 ml-1"><span className="text-orange-400 mt-1.5 shrink-0 text-xs">●</span><span>{renderInline(line.slice(2))}</span></div>;
        if (line.match(/^\d+\.\s/)) {
          const dot = line.indexOf(". ");
          return <div key={i} className="flex gap-2.5 ml-1"><span className="text-orange-400 font-bold shrink-0 w-5 mt-0.5 text-sm">{line.slice(0, dot)}.</span><span>{renderInline(line.slice(dot + 2))}</span></div>;
        }
...
function GenerateModal({
  websites, connections, generating,
  onGenerate, onClose,
}: {
  websites: Website[];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          {/* Period */}
          <div>
            <p className="text-xs text-zinc-600 uppercase tracking-wider mb-2">Davr bo'yicha</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                { key: "weekly",  label: "Haftalik",  sub: "7 kun",   endpoint: "/ai/insights/weekly",  Icon: BarChart2,    color: "text-orange-400 border-orange-500/20 bg-orange-500/5 hover:bg-orange-500/10" },
                { key: "monthly", label: "Oylik",     sub: "30 kun",  endpoint: "/ai/insights/monthly",  Icon: CalendarDays, color: "text-yellow-400 border-yellow-500/20 bg-yellow-500/5 hover:bg-yellow-500/10" },
                { key: "yearly",  label: "Yillik",    sub: "365 kun", endpoint: "/ai/insights/yearly",   Icon: Calendar,     color: "text-purple-400 border-purple-500/20 bg-purple-500/5 hover:bg-purple-500/10" },
              ].map(({ key, label, sub, endpoint, Icon, color }) => (
                <button
                  key={key}
                  disabled={!!generating}
                  onClick={() => { onGenerate(endpoint, key); onClose(); }}
                  className={cn("flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all disabled:opacity-40", color)}
                >
                  {generating === key
          ...
          {websites.length > 0 && (
            <div>
              <p className="text-xs text-zinc-600 uppercase tracking-wider mb-2">Sayt tahlili</p>
              <div className="space-y-1">
                {websites.map((w) => (
                  <button
                    key={w.id}
                    disabled={!!generating}
                    onClick={() => { onGenerate(`/ai/insights/website/${w.id}`, `w-${w.id}`); onClose(); }}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      {w.domain && <p className="text-xs text-zinc-600 truncate">{w.domain}</p>}
                    </div>
                  </button>
                ))}
              </div>
          ...
          {/* Platforms */}
          {connections.filter(c => ["YOUTUBE","TELEGRAM","INSTAGRAM","DISCORD","BLUESKY"].includes(c.platform)).length > 0 && (
            <div>
              <p className="text-xs text-zinc-600 uppercase tracking-wider mb-2">Platforma tahlili</p>
              <div className="space-y-1">
                {connections
                  .filter(c => ["YOUTUBE","TELEGRAM","INSTAGRAM","DISCORD","BLUESKY"].includes(c.platform))
                  .map((c) => (
                    <button
                      key={c.platform}
                      disabled={!!generating}
                      onClick={() => { onGenerate(`/ai/insights/platform/${c.platform}`, `p-${c.platform}`); onClose(); }}
                    ...
                    >
                      {generating === `p-${c.platform}`
                        ? <div className="w-4 h-4 rounded-full border-2 border-green-400 border-t-transparent animate-spin shrink-0" />
                        : <Link2 className="w-4 h-4 shrink-0" />}
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-zinc-200 truncate">{c.platform}</p>
                        {c.platformUsername && <p className="text-xs text-zinc-600">@{c.platformUsername}</p>}
                      </div>
                    </button>
                  ))}
              </div>
...
function InsightDetail({
  ...
  const [platHistories, setPlatHistories] = useState<
    Record<string, { history: PlatformHistoryPoint[]; username: string | null }>
  >({});
  ...
  // Platform name for CUSTOM type (from title)
  const platformName = insight.type === "CUSTOM"
    ? insight.title?.replace(" tahlili", "").trim()?.toUpperCase()
    : null;

  const isPeriod = ["WEEKLY", "MONTHLY", "YEARLY"].includes(insight.type);
  const days = insight.type === "YEARLY" ? 365 : insight.type === "MONTHLY" ? 30 : 14;
  ...
  useEffect(() => {
    ...
    if (isPeriod) {
      // Web trend
      dashboardApi.webTrend(days).then(setWebTrend).catch(() => {});
      // All connected platform histories
      connections.forEach((c) => {
        dashboardApi.platformHistory(c.platform, days)
          .then((h) => {
            if (h?.history?.length) {
              setPlatHistories((prev) => ({
                ...prev,
                [c.platform]: { history: h.history, username: h.username },
              }));
            }
          })
          .catch(() => {});
      });
    } else if (platformName && PLATFORM_CHART_COLORS[platformName]) {
      // Single platform history for CUSTOM type
      dashboardApi.platformHistory(platformName, 30)
        .then((h) => {
          if (h?.history?.length) {
            setPlatHistories({ [platformName]: { history: h.history, username: h.username } });
          }
        })
        .catch(() => {});
    } else if (insight.type === "WEBSITE") {
      // Web trend for website insights
      dashboardApi.webTrend(30).then(setWebTrend).catch(() => {});
    }
  }, [insight.id]);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 pb-5 border-b border-zinc-800/60 mb-6">
        <div className={cn("w-9 h-9 rounded-xl bg-zinc-800/80 border border-zinc-700/50 flex items-center justify-center shrink-0", color)}>
          <Icon className="w-4.5 h-4.5" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-base font-semibold text-zinc-100">{insight.title ?? TYPE_LABELS[insight.type]}</p>
          <p className="text-xs text-zinc-500 mt-0.5">
            ...
            {" · "}{insight.provider}
            {insight.tokensUsed ? ` · ${insight.tokensUsed.toLocaleString()} token` : ""}
          </p>
        </div>
        <button
      ...
      {(webTrend.length > 1 || platEntries.length > 0) && (
        <div className="mb-6 rounded-xl bg-zinc-900/60 border border-zinc-800/60 overflow-hidden">
          ...
          {webTrend.length > 1 && (
            <div className="p-4 border-b border-zinc-800/60">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs text-zinc-500">Sayt tashriflari ({days} kun)</p>
                <p className="text-sm font-bold text-zinc-100">{totalViews.toLocaleString()}</p>
              </div>
              <ResponsiveContainer width="100%" height={64}>
                <AreaChart data={webTrend.map((t) => ({ ...t, date: fmtDate(t.date) }))}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="views" stroke="#f97316" strokeWidth={1.5} fill="url(#cg)" dot={false} activeDot={{ r: 3 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
          ...
          {platEntries.length > 0 && (
            <div className={`grid p-4 gap-4 ${platEntries.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>
              {platEntries.map(([platform, { history, username }]) => {
                const color = PLATFORM_CHART_COLORS[platform] ?? "#f97316";
                const latest = history[history.length - 1];
                const first = history[0];
                const growth = first?.followers && latest?.followers
                  ...
                  <div key={platform}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div>
                        ...
                        {username && <p className="text-[10px] text-zinc-600">@{username}</p>}
                      </div>
                      <div className="text-right">
                        ...
                        {growth && (
                          <p className={`text-[10px] flex items-center gap-0.5 justify-end ${Number(growth) >= 0 ? "text-green-400" : "text-red-400"}`}>
                            {Number(growth) >= 0 ? <TrendingUp className="w-2.5 h-2.5" /> : <TrendingDown className="w-2.5 h-2.5" />}
                    ...
                    <ResponsiveContainer width="100%" height={52}>
                      <LineChart data={history.map((h) => ({ ...h, date: fmtDate(String(h.date)) }))}>
                        <XAxis dataKey="date" tick={{ fill: "#52525b", fontSize: 9 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTip />} />
                        <Line type="monotone" dataKey="followers" stroke={color} strokeWidth={2} dot={false} activeDot={{ r: 3 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                );
...
function WelcomeState({ onNew }: { onNew: () => void }) {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center px-8">
      ...
      <h2 className="text-lg font-semibold text-zinc-100 mb-2">AI tahlil tayyor</h2>
      <p className="text-sm text-zinc-500 mb-6 max-w-xs leading-relaxed">
        Haftalik, oylik yoki yillik tahlil — platformalar va saytlaringiz haqida amaliy maslahat oling
      </p>
...
export default function InsightsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiClient.get<AiInsight[]>("/ai/insights", { params: { limit: 50 } });
  ...
  async function generate(endpoint: string, key: string) {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? "AI xatosi. API key tekshiring.");
    } finally {
      setGenerating(null);
    }
  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className={cn("text-xs font-medium truncate", isActive ? "text-zinc-100" : "text-zinc-300")}>
