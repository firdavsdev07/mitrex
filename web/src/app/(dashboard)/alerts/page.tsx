...
interface Alert {
  ...
  connection: { id: string; platform: string; platformUsername: string | null } | null;
...
const PLATFORM_LABELS: Record<string, string> = {
  YOUTUBE: "YouTube", TELEGRAM: "Telegram", INSTAGRAM: "Instagram",
  DISCORD: "Discord", BLUESKY: "Bluesky",
};
...
export default function AlertsPage() {
  ...
  async function toggleAlert(id: string, current: boolean) {
    ...
    setAlerts((prev) => prev.map((a) => (a.id === id ? { ...a, isActive: !current } : a)));
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-xs text-zinc-600 uppercase tracking-wider mb-0.5">Ogohlantirishlar</p>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-zinc-800 border-dashed bg-zinc-900/30 p-12 text-center">
          ...
          <p className="text-xs text-zinc-700 mb-5">
            Traffic tushib ketsa yoki obunachi keskin o&apos;ssa — siz birinchi bilasiz
          </p>
          <Button onClick={() => setAddModal(true)} className="gap-1.5">
            <Plus className="w-4 h-4" />
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <p className="text-sm font-semibold text-zinc-100">{alert.name}</p>
                      <Badge variant={alert.isActive ? "success" : "default"}>
                        {alert.isActive ? "Faol" : "O'chirilgan"}
                      </Badge>
                      <Badge variant={alert.channel === "EMAIL" ? "default" : "orange"}>
                        {alert.channel === "EMAIL" ? (
                          <span className="flex items-center gap-1"><Mail className="w-3 h-3" />Email</span>
                        ) : (
                          <span className="flex items-center gap-1"><BellRing className="w-3 h-3" />In-app</span>
                        )}
                      </Badge>
                    </div>
                    ...
                    <p className="text-xs text-zinc-500">
                      ...
                      {" · "}Chegara: <span className="text-zinc-300">{alert.threshold}x</span>
                    </p>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      ...
                      {alert.connection && (
                        <span className="flex items-center gap-1 text-xs text-zinc-600">
                          ...
                          {PLATFORM_LABELS[alert.connection.platform] ?? alert.connection.platform}
                          {alert.connection.platformUsername && ` @${alert.connection.platformUsername}`}
                        </span>
                      )}
                      {alert.lastTriggered && (
                        <span className="text-xs text-zinc-700">
                          Oxirgi: {new Date(alert.lastTriggered).toLocaleDateString("uz-UZ")}
                        </span>
                      )}
                    </div>
                  </div>
...
function AddAlertModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    if (targetType === "website" && !websiteId) { setError("Sayt tanlang"); return; }
    if (targetType === "connection" && !connectionId) { setError("Platforma tanlang"); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-xl border border-zinc-800 bg-zinc-900 p-5 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-zinc-100">Yangi alert</h3>
