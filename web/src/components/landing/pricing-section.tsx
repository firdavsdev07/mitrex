...
import { Check, X, Globe, Zap, Bot, Bell, Download, Users, Key, Database, BarChart2, Link2 } from "lucide-react";
  ...
  },
  {
    ...
    groups: [
      ...
      },
      {
        label: "Platformalar",
        features: [
          { icon: Link2,    text: "Barcha 8 ta platforma",       included: true, highlight: true },
          { icon: Database, text: "1 yillik ma'lumot tarixi", included: true, highlight: true },
        ],
      },
      {
        label: "AI & Alertlar",
        features: [
          { icon: Bot,  text: "AI haftalik hisobot (email)", included: true, highlight: true },
    ...
    groups: [
      {
        label: "Tracking",
        features: [
          { icon: Globe,     text: "Cheksiz sayt",          included: true, highlight: true },
          { icon: Zap,       text: "Cheksiz tashrif",       included: true, highlight: true },
          { icon: BarChart2, text: "Real vaqt monitoring",  included: true  },
        ],
      },
      {
        ...
        features: [
          ...
          { icon: Database, text: "Cheksiz ma'lumot tarixi",  included: true, highlight: true },
        ],
      },
      {
        label: "AI & Alertlar",
        features: [
          { icon: Bot,  text: "AI haftalik + custom hisobot", included: true, highlight: true },
          { icon: Bell, text: "Custom alertlar",              included: true, highlight: true },
        ],
      },
      {
        label: "Jamoa & Eksport",
        features: [
          { icon: Users,    text: "Workspace + ruxsatlar",  included: true, highlight: true },
          { icon: Download, text: "CSV eksport",            included: true  },
...
/* ── Feature row ─────────────────────────────────── */
function FeatureRow({ feature, featured }: { feature: Feature; featured: boolean }) {
  const Icon = feature.icon;
  if (!feature.included) {
    return (
      <li className="flex items-center gap-2.5 py-1">
        <X className="w-3.5 h-3.5 text-zinc-700 shrink-0" />
        <span className="text-xs text-zinc-600 line-through">{feature.text}</span>
      </li>
    );
  }
  return (
    <li className="flex items-center gap-2.5 py-1">
      ...
      <span className={cn(
        ...
        feature.highlight && featured ? "text-zinc-200 font-medium" : "text-zinc-400"
      )}>
        {feature.text}
      </span>
    </li>
...
function PlanCard({ plan, index }: { plan: typeof plans[0]; index: number }) {
  const allFeatureCount = plan.groups.reduce((a, g) => a + g.features.filter(f => f.included).length, 0);
    ...
    >
      ...
      <div className={cn(
        ...
        <p className="text-[11px] text-zinc-600 mt-3">
          <span className="text-zinc-400 font-semibold">{allFeatureCount}</span> ta imkoniyat kiradi
        </p>
      </div>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        >
          ...
          <p className="text-sm text-zinc-500 max-w-md mx-auto leading-relaxed">
            Bepul boshlab, o'sgach yangilaysiz. Istalgan vaqt bekor qilish mumkin.
          </p>
        </div>
        ...
        >
          {["Kredit karta kerak emas", "Istalgan vaqt bekor qilish", "Ma'lumotlar xavfsiz"].map((t) => (
        ...
        >
          Savol bormi?{" "}
          <a href="mailto:hello@metrix.io" className="text-zinc-500 hover:text-zinc-300 transition-colors">
            hello@metrix.io
          </a>{" "}
          ga yozing
        </p>
