...
import { Globe, Bot, Zap, Bell, Download, Code2, BarChart3, Monitor, Smartphone } from "lucide-react";
    ...
    <div className="mt-5 space-y-3">
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3 overflow-hidden">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] font-semibold text-green-400">↑ 23%</span>
        </div>
        <svg viewBox="0 0 220 52" className="w-full" preserveAspectRatio="none" style={{ height: 52 }}>
          <defs>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800">
          <span className="text-[10px] font-medium text-zinc-400">Top sahifalar</span>
          <span className="text-[10px] text-zinc-600">Ko'rishlar</span>
        </div>
        {pages.map((p) => (
          <div key={p.path} className="px-3 py-2 border-b border-zinc-800/40 last:border-0 group hover:bg-zinc-800/20 transition-colors">
            <div className="flex justify-between mb-1.5">
              <span className="text-[10px] text-zinc-400 font-mono">{p.path}</span>
              <span className="text-[10px] font-semibold text-zinc-300">{p.views}</span>
            </div>
  ...
  return (
    <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-900/80 overflow-hidden">
      ...
      <div className="px-3 py-2.5 border-b border-zinc-800 bg-zinc-900">
        ...
        <p className="text-[11px] font-semibold text-zinc-200">Haftalik tahlil — 16-22 Iyun</p>
      </div>
      {/* Email body */}
      <div className="p-3 space-y-2">
        <div className="flex items-start gap-2">
          <span className="text-orange-400 text-[10px] font-bold shrink-0">↑</span>
          <p className="text-[10px] text-zinc-400 leading-relaxed">Saytga 1,247 tashrif — o'tgan haftadan <span className="text-green-400 font-medium">+12%</span></p>
        </div>
        <div className="flex items-start gap-2">
          <span className="text-orange-400 text-[10px] font-bold shrink-0">↑</span>
          <p className="text-[10px] text-zinc-400 leading-relaxed">YouTube: <span className="text-green-400 font-medium">+145</span> yangi obunachi</p>
        </div>
        <div className="flex items-start gap-2">
          <span className="text-orange-400 text-[10px] font-bold shrink-0">!</span>
          <p className="text-[10px] text-zinc-400 leading-relaxed">Bounce rate <span className="text-yellow-400 font-medium">68%</span> — landing page ni optimallashtiring</p>
        </div>
        <div className="mt-2 pt-2 border-t border-zinc-800">
          <p className="text-[10px] font-semibold text-orange-400">Keyingi hafta uchun maslahat:</p>
          <p className="text-[10px] text-zinc-500 mt-0.5">Chorshanba va Shanba kuni post yuboring — bu kunlar eng faol</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      <div className="flex items-center gap-4 p-3 rounded-xl border border-green-500/15 bg-green-500/5">
        <div className="relative">
          <div className="w-14 h-14 rounded-full border border-green-500/20 bg-green-500/8 flex items-center justify-center">
            <span className="text-2xl font-black text-green-400 tabular-nums">12</span>
          </div>
          <span
      ...
      <div className="space-y-1.5">
        {pages.map((p) => (
          <div key={p.path} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-zinc-800/60 bg-zinc-900/40">
            <div className="w-1.5 h-1.5 rounded-full bg-green-500" style={{ animation: "ping-slow 2s ease-in-out infinite" }} />
            <span className="text-[10px] text-zinc-400 font-mono flex-1">{p.path}</span>
            <span className="text-[10px] font-bold text-zinc-300">{p.count}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-[#0d0d12] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800/80 bg-zinc-900/60">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-600 font-mono ml-1">index.html</span>
          </div>
        ...
        <div className="flex font-mono text-[11px] leading-6 p-3 gap-3">
          ...
          <div className="flex flex-col items-end select-none shrink-0">
            {lines.map((l) => (
              <span key={l.num} className="text-zinc-700 text-[10px] leading-6">{l.num}</span>
            ))}
          </div>
          {/* Code */}
...
function AlertsVisual() {
  return (
    <div className="mt-4 space-y-2">
      {[
        { color: "green",  title: "Traffic spike!",  body: "Saytingiz trafigi 2x oshdi — 847 tashrif/soat", time: "hozir" },
        { color: "yellow", title: "Yangi manba",     body: "Twitter orqali 45 ta tashrif kelyapti", time: "5 min" },
        { color: "red",    title: "Traffic drop",    body: "Telegram kanalida faollik 40% kamaydi", time: "1 soat" },
      ].map((a) => {
        ...
        return (
          <div key={a.title} className={`flex items-start gap-2.5 p-2.5 rounded-xl border ${colors[a.color]}`}>
            <div className={`w-1.5 h-1.5 rounded-full mt-1 shrink-0 ${dots[a.color]}`} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                <p className="text-[10px] font-semibold truncate">{a.title}</p>
                <span className="text-[9px] text-zinc-700 shrink-0">{a.time}</span>
              </div>
              <p className="text-[10px] text-zinc-500 mt-0.5 leading-relaxed">{a.body}</p>
            </div>
          </div>
        );
      })}
    ...
    <div className="mt-4 space-y-2.5">
      <div className="rounded-xl border border-zinc-800 overflow-hidden bg-zinc-900/60">
        <div className="flex gap-0 text-[9px] font-mono border-b border-zinc-800">
          {["date","path","views","device"].map((h) => (
            <div key={h} className="flex-1 px-2 py-1.5 text-zinc-500 border-r border-zinc-800 last:border-0 bg-zinc-800/30">{h}</div>
          ))}
        </div>
        {[
          ...
          <div key={i} className="flex gap-0 text-[9px] font-mono border-b border-zinc-800/40 last:border-0">
            {row.map((cell, j) => (
              <div key={j} className="flex-1 px-2 py-1.5 text-zinc-500 border-r border-zinc-800/30 last:border-0 truncate">{cell}</div>
            ))}
          </div>
        ))}
      </div>
      <div className="flex gap-1.5">
        {["Excel", "Google Sheets", "CSV"].map((f) => (
          <span key={f} className="text-[10px] px-2 py-0.5 rounded-md border border-zinc-700/60 text-zinc-500">{f}</span>
        ))}
      </div>
    </div>
  );
...
const accentStyles: Record<string, { icon: string; border: string; label: string }> = {
  orange: { icon: "text-orange-400 bg-orange-500/10 border-orange-500/20", border: "hover:border-orange-500/25", label: "text-orange-500/70" },
  green:  { icon: "text-green-400  bg-green-500/10  border-green-500/20",  border: "hover:border-green-500/25",  label: "text-green-500/70"  },
  blue:   { icon: "text-blue-400   bg-blue-500/10   border-blue-500/20",   border: "hover:border-blue-500/25",   label: "text-blue-500/70"   },
  yellow: { icon: "text-yellow-400 bg-yellow-500/10 border-yellow-500/20", border: "hover:border-yellow-500/25", label: "text-yellow-500/70" },
  zinc:   { icon: "text-zinc-400   bg-zinc-800      border-zinc-700",      border: "hover:border-zinc-600",      label: "text-zinc-500"      },
};
  ...
  return (
    <section id="imkoniyatlar" className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        {/* Heading */}
        <div className="max-w-xl mb-12" style={{ animation: "fade-up 0.6s ease-out both" }}>
          ...
          {bento.map((card, i) => {
            ...
            return (
              <div
                ...
                style={{ animation: `fade-up 0.6s ease-out ${0.08 + i * 0.07}s both` }}
              >
                {/* Hover shimmer */}
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <p className={`text-[10px] uppercase tracking-widest font-medium mb-1.5 ${a.label}`}>
                      {card.label}
                    </p>
                  ...
                  <div className={`w-8 h-8 rounded-xl border flex items-center justify-center shrink-0 ${a.icon}`}>
