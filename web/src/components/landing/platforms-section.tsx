...
function HTMLCode() {
  return (
    <>
      ...
      <L n={2}>{pun("<")}{tag("script")}</L>
      <L n={3}>{txt("  ")}{atr("src")}{pun("=")}{str('"https://app.metrix.io/track.js"')}</L>
      <L n={4}>{txt("  ")}{atr("data-site")}{pun("=")}{val('"mk_live_abc123"')}</L>
      <L n={5}>{txt("  ")}{kw("defer")}</L>
      <L n={6}>{pun("></")}{tag("script")}{pun(">")}</L>
    </>
  );
}
...
function ReactCode() {
  return (
    <>
      <L n={1}>{kw("import")}{txt(` { useEffect } `)}{kw("from")}{txt(" ")}{str("'react'")}</L>
      <L n={2} />
      <L n={3}>{fn("useEffect")}{pun("(")}{pun("()")}{txt(" => {")}</L>
      <L n={4}>{txt("  ")}{kw("const")}{txt(" s = document.")}{fn("createElement")}{pun("(")}{str("'script'")}{pun(")")}</L>
      <L n={5}>{txt("  s.")}{atr("src")}{txt(" = ")}{str('"https://app.metrix.io/track.js"')}</L>
      <L n={6}>{txt("  s.dataset.")}{atr("site")}{txt(" = ")}{val('"mk_live_abc123"')}</L>
      <L n={7}>{txt("  s.")}{atr("defer")}{txt(" = ")}{kw("true")}</L>
      <L n={8}>{txt("  document.body.")}{fn("appendChild")}{pun("(")}{txt("s")}{pun(")")}</L>
      <L n={9}>{pun("}, ")}{pun("[]")}{pun(")")}</L>
    </>
  );
}
...
function NextCode() {
  return (
    <>
      <L n={1}>{kw("import")}{txt(" Script ")}{kw("from")}{txt(" ")}{str("'next/script'")}</L>
      <L n={2} />
      <L n={3}>{kw("export default function")}{txt(" ")}{fn("Layout")}{pun("({")}{txt(" children ")}{pun("}){")} </L>
      <L n={4}>{txt("  ")}{kw("return")}{txt(" (")}</L>
      <L n={5}>{txt("    ")}{pun("<")}{tag("html")}{pun(">")}</L>
      <L n={6}>{txt("      ")}{pun("<")}{tag("body")}{pun(">")}{pun("{")}{txt("children")}{pun("}")}{pun("</")}{tag("body")}{pun(">")}</L>
      <L n={7}>{txt("      ")}{pun("<")}{tag("Script")}</L>
      <L n={8}>{txt("        ")}{atr("src")}{pun("=")}{str('"https://app.metrix.io/track.js"')}</L>
      <L n={9}>{txt("        ")}{atr("data-site")}{pun("=")}{val('"mk_live_abc123"')}</L>
      <L n={10}>{txt("        ")}{atr("strategy")}{pun("=")}{str('"afterInteractive"')}</L>
      <L n={11}>{txt("      ")}{pun("/>")}</L>
      <L n={12}>{txt("    ")}{pun("</")}{tag("html")}{pun(">")}</L>
      <L n={13}>{txt("  )")}</L>
      <L n={14}>{pun("}")}</L>
    </>
  );
...
function VueCode() {
  return (
    <>
      <L n={1}>{cmt("// main.ts")}</L>
      <L n={2}>{kw("import")}{txt(" { createApp } ")}{kw("from")}{txt(" ")}{str("'vue'")}</L>
      <L n={3}>{kw("import")}{txt(" App ")}{kw("from")}{txt(" ")}{str("'./App.vue'")}</L>
      <L n={4} />
      <L n={5}>{cmt("// Metrix tracking")}</L>
      <L n={6}>{kw("const")}{txt(" s = document.")}{fn("createElement")}{pun("(")}{str("'script'")}{pun(")")}</L>
      <L n={7}>{txt("s.")}{atr("src")}{txt(" = ")}{str('"https://app.metrix.io/track.js"')}</L>
      <L n={8}>{txt("s.")}{fn("setAttribute")}{pun("(")}{str("'data-site'")}{pun(", ")}{val("'mk_live_abc123'")}{pun(")")}</L>
      <L n={9}>{txt("document.head.")}{fn("appendChild")}{pun("(")}{txt("s")}{pun(")")}</L>
      <L n={10} />
      <L n={11}>{fn("createApp")}{pun("(")}{txt("App")}{pun(").")}{fn("mount")}{pun("(")}{str("'#app'")}{pun(")")}</L>
    </>
  );
}
...
function CurlCode() {
  return (
    <>
      ...
      <L n={2}>{fn("curl")}{txt(" -X ")}{val("POST")}{txt(" ")}{str("https://app.metrix.io/track")}{txt(" \\")}</L>
      <L n={3}>{txt("  ")}{txt("-H ")}{str('"Content-Type: application/json"')}{txt(" \\")}</L>
      <L n={4}>{txt("  ")}{txt("-d ")}{str("'{")} </L>
      <L n={5}>{txt("    ")}{str('"siteKey"')}{pun(": ")}{val('"mk_live_abc123"')}{pun(",")}</L>
      <L n={6}>{txt("    ")}{str('"path"')}{pun(": ")}{val('"/home"')}{pun(",")}</L>
      <L n={7}>{txt("    ")}{str('"sessionId"')}{pun(": ")}{val('"sess_xyz789"')}</L>
      <L n={8}>{txt("  ")}{str("}'")}</L>
    </>
  );
}
...
/* ── Tab config ──────────────────────────────────── */
const tabs: Array<{ id: string; file: string; lang: string; Code: () => React.ReactElement; raw: string }> = [
  {
    id: "HTML",
    file: "index.html",
    lang: "HTML",
    ...
    <section className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        >
          ...
          {coming.map((p) => (
            <span key={p} className="text-[11px] text-zinc-700 px-2 py-0.5 rounded-md border border-zinc-800">
              {p}
            </span>
          ))}
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
            <div>
              <p className="text-sm font-semibold text-zinc-100">Saytingizni ulang</p>
              <p className="text-xs text-zinc-500 mt-0.5">Framework tanlang — kod avtomatik o'zgaradi</p>
            </div>
                ...
                return (
                  ...
                  >
                    <div className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors ${
                      isActive ? "bg-orange-500" : "bg-zinc-700 group-hover:bg-zinc-500"
                    }`} />
                    <div>
                      <p className="text-[13px] font-semibold leading-none">{tab.id}</p>
            ...
            <div className="bg-[#0c0c10] flex flex-col">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-800/60 bg-zinc-900/30 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] text-zinc-600 font-mono">{active.file}</span>
                </div>
              ...
              <div className="px-5 pb-4 border-t border-zinc-800/40 pt-3">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-zinc-800 bg-zinc-900/60">
                  <span className="text-[10px] text-zinc-600 font-mono">data-site:</span>
                  <span className="text-[11px] font-mono font-bold text-orange-400">mk_live_abc123</span>
                  <span className="text-[10px] text-zinc-700">← o'zingizniki bilan almashtiring</span>
                </div>
              </div>
            </div>
          </div>
